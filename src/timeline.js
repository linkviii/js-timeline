...
class Timeline {
    ...
    add_axis_label(dt, label, kw) {
        //date, string?
        kw = kw || {};
        if (this.tick_format) {
        }
        const percent_width = (dt.valueOf() - this.date0) / 1000 / this.total_secs;
    ...
    create_callouts() {
        ...
        for (let callout of callouts_data) {
            ...
            const newInfo = [event, event_color];
            const events = inv_callouts.get(event_date);
            events.push(newInfo);
        }
        sorted_dates.sort();
        ...
        for (let event_date of sorted_dates) {
            const [event, event_color] = inv_callouts.get(event_date).pop();
            const num_sec = (event_date - this.date0) / 1000;
            const percent_width = num_sec / this.total_secs;
            if (percent_width < 0 || percent_width > 1) {
                continue;
            }
            const x = Math.trunc(percent_width * this.width + 0.5);
            ...
            const y = 0 - this.callout_size[1] - k * this.callout_size[2];
            min_y = Math.min(min_y, y);
            //path_data = 'M%i,%i L%i,%i L%i,%i'
            // % (x, 0, x, y, x - self.callout_size[0], y)
            const path_data = 'M' + x + ',' + 0 + ' L' + x + ',' + y + ' L'
            ...
            this.add_axis_label(eDate, eDate.toLocaleString(), { tick: false, fill: Colors.black });
